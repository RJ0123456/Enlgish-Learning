...
function bindWordEvents(){document.querySelectorAll('.word').forEach(element=>{element.addEventListener('click',event=>showPopover(element,event));element.addEventListener('dblclick',()=>{const word=element.dataset.word;if(!word||state.customWords.some(item=>item.word===word)||presetWords.some(item=>item.word===word))return;state.customWords.push({word,meaning:getWordMeaning(word)});renderVocab();showToast('已加入我的生词')})})}
