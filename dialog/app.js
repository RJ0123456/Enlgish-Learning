...
const state = {scenes:[], current:0, customWords:[], fontSize:17, audio:null, recorder:null, recordingChunks:[], recordingUrl:null};
...
function bindWordEvents(){document.querySelectorAll('.word').forEach(element=>{element.addEventListener('click',event=>{if(state.audio&&!state.audio.paused){state.audio.pause();event.stopPropagation();return}showPopover(element,event);event.stopPropagation()});element.addEventListener('dblclick',()=>{const word=element.dataset.word;if(!word||state.customWords.some(item=>item.word===word)||presetWords.some(item=>item.word===word))return;state.customWords.push({word,meaning:getWordMeaning(word)});renderVocab();showToast('已加入我的生词')})})}
function showPopover(element,event){const word=element.dataset.word;$('#popover-word').textContent=word;$('#popover-meaning').textContent=getWordMeaning(word);const popover=$('#word-popover');popover.classList.add('visible');popover.setAttribute('aria-hidden','false');popover.style.left=`${Math.min(event.clientX,window.innerWidth-205)}px`;popover.style.top=`${Math.min(event.clientY+15,window.innerHeight-150)}px`;$('#speak-word').onclick=()=>speak(word)}
function showToast(message){const toast=$('#toast');toast.textContent=message;toast.classList.add('visible');setTimeout(()=>toast.classList.remove('visible'),1600)}
function clearRecording(){if(state.recorder&&state.recorder.state!=='inactive')state.recorder.stop();state.recorder=null;state.recordingChunks=[];if(state.recordingUrl)URL.revokeObjectURL(state.recordingUrl);state.recordingUrl=null;$('#recording-panel').hidden=true;$('#record-button').classList.remove('recording');$('#record-label').textContent='开始录音';$('.record-icon').textContent='●'}
...
$('#dialogue').addEventListener('click',event=>{if(!state.audio||!state.audio.src)return;if(event.target.closest('.word'))return;if(!state.audio.paused){state.audio.pause()}else{state.audio.play().catch(()=>{})}});
